...
function Events(props) {
    ...
    const eventsAPI = useEventsPageInfoQuery();
    ...
    useEffect(() => {
        if (eventsAPI.status === "fulfilled") {
            if (eventsAPI?.data?.status) {
                setEventList(eventsAPI?.data?.info);
            }
        }
    }, [eventsAPI])
    return (
        <>
            ...
            <section>
                <div className='news-banner-section mb-2 Event-banner-m'>
                    <img className='block-chain-img' src={EventBanner} />
                    <div className='Event-banner-l'>
                        <h2>Upcoming Events</h2>
                        <h1>AMH ART PRODUCT EVENT</h1>
            ...
            <section className='news-section abt-main-box'>
                <Container className=''>
                    <h3 className='abt-head mt-3 mb-5'>Events</h3>
                    <br />
                    <br />
                    <div className='row'>
                                                    ...
                                                    switch (Cname) {
                                                        case 0:
                                                            return <div className='col-lg-4 '>
                                                                <a className='' onClick={() => openModal({ Name: event?.Name, Image: event?.Image, Info: event?.Info })}>
                                                                    <div className="collectionTwo image-container">
                                                                        <img src={event?.Image} className="artistOne" />
                                                                        <div className="image-overlay">
                                                                            <div>{event?.Name}
                                                                                <p className="image-text">{event?.PublishDate}</p>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </a>
                                                            ...
                                                            return <div className='col-lg-4 '>
                                                                <a className='' onClick={() => openModal({ Name: event?.Name, Image: event?.Image, Info: event?.Info })}>
                                                                    <div className="image-container image-container-artistTwo">
                                                                        <img src={event?.Image} className="artistTwo" />
                                                                        <div className="image-overlay">
                                                                            <div>{event?.Name}
                                                                                <p className="image-text">{event?.PublishDate}</p>
                                                                            </div>
