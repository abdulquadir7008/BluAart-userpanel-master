...
function CollectionART(props) {
  ...
  const navCreatecolleciton = () => {
    navigate(`/editcollection/${encodeURIComponent(parms?.id)}`)
  }
  ...
  const navCreateItem = (type) => {
    if (type === "bulkartproduct") {
      navigate(`/create-item/${encodeURIComponent(parms?.id)}/bulk-art-product`)
    } else {
      navigate(`/create-item/${encodeURIComponent(parms?.id)}/${type}`)
    }
  }
  const navArtProdcutcreateItem = () => {
    navigate(`/create-item/${encodeURIComponent(parms?.id)}/art-product`)
  }
  ...
  const mint = async (contractAddress, quantity, collectionitem, collectionId) => {
    ...
    for (let index = 0; index < collectionitem.length; index++) {
      if (collectionitem[index]?.ApproveStatus) {
        TokenId++;
    ...
    if (itemIds?.length > 0 && tokenIds?.length > 0) {
      ...
      showErroToast("Admin has to approve item before minting")
  ...
  useEffect(() => {
    ...
    return () => {
      socket.off('ArtBulkItemMint');
      socket.off('ArtItemBulkPublish');
    };
  }, [])
  return (
    <div>
        ...
        <Container className='section-item'>
          <div className='auction-tab-section'>
            <div className='row artist' >
              {sessionStorage.getItem('isWalletConnected') !== "true" && sessionStorage.getItem('login') !== "true" && collectionItemList?.length > 0 && collectionItemList.map((item, index) => {
                let Cname = index % 3;
            ...
            <Tabs defaultActiveKey="tab1">
              ...
              <TabPanel eventKey="tab2">
                <div className='d-flex justify-content-end '>
                  <button className='btn btn-primary' disabled={mintLoadint} onClick={() => bulkMint(
                    collectionInfo[0]?.CollectionInfo.Currency,
                    collectionInfo[0].CollectionInfo?.ContractAddress,
                    collectionInfo[0].CollectionInfo?._id
                  )}>Bulk MINT</button>
                </div>
                <br />
                <br />
                <div className='row artist' >
                  {collectionUnMintedItemList?.length > 0 && collectionUnMintedItemList.map((item, index) => {
                    let Cname = index % 3;
                    const encryptedItemId = CryptoJS.AES.encrypt(item._id.toString(), process.env.REACT_APP_SECRET_PASS).toString();
